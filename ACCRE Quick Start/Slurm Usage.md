...
4. Submitting GPU jobs is a little bit different on R9. For GPU jobs, the account is the same one as you used for GPU jobs on CentOS & cluster, but the partition should be `batch_gpu`, and you also need to specify the GRES type you need (e.g. `--gres=gpu:nvidia_geforce_rtx_2080_ti:1`)\
