# Slurm Usage
## Table of Contents
- [Notes for R9 Cluster](#Notes-for-R9-Cluster) **<- READ THIS FIRST**
...
## Notes for R9 Cluster
***Updated on: 05/19/2025***

As you may have noticed, ACCRE is now upgrading to Rocky 9 (R9) cluster (if you are logging in with `login.accre.vu` you will be on R9 cluster), which has several differences compared to the the legacy CentOS 7 cluster (`login.accre.vanderbilt.edu` or `maizie1/2/3.accre.vanderbilt.edu`).

While most of the changes has been well documented [HERE](https://help.accre.vanderbilt.edu/index.php?title=SLURM), there are still some items that need to be highlighted:

1. Currently, maizie nodes are still on CentOS cluster, which means if you want to submit jobs to maizie nodes you still need to login with `login.accre.vanderbilt.edu` or `maizie1/2/3.accre.vanderbilt.edu`
2. [Legacy Visualization Portal](https://portal.accre.vanderbilt.edu/) will submit jobs to the legacy CentOS 7 cluster. The [new Visualization Portal](https://viz.accre.vu/) will connect to the Rocky 9 cluster environment.
3. R9 cluster has two patitions for non-GPU jobs: `production` and `batch`, `batch` is the default one if the partition is not specified. Both can be accessed with the account you used to access `production` partition on CentOS 7 cluster before. `batch` partition has the same configuration with the legacy `production` partition for compatibility. For more information, please see [here](https://help.accre.vanderbilt.edu/index.php?title=SLURM#New_Partitions_in_the_R9_Cluster)
4. Submitting GPU jobs is a little bit different on R9. For GPU jobs, the account is the same one as you used for GPU jobs on CentOS 7 cluster, but the partition should be `batch_gpu`, and you also need to specify the GRES type you need (e.g. `--gres=gpu:nvidia_geforce_rtx_2080_ti:1`)\
Here's a example script for requesting interactive GPU node resources: `salloc --account=maiziezhou_lab_acc --partition=batch_gpu --mem=8G --time=00-01:00:00 --gres=gpu:nvidia_geforce_rtx_2080_ti:1`, you can also refer to this to modify the corresponding parameters in slurm script. For more information, please see [here](https://help.accre.vanderbilt.edu/index.php?title=GPUs_at_ACCRE)
